...
public class TT1ActionProvider
 {
	...
	public static JSONObject actionKeHoach(ThemeDisplay themeDisplay, UploadPortletRequest uploadPortletRequest,
		...
		int documentName = ParamUtil.getInteger(uploadPortletRequest,
				ChuyenDichTrangThaiUtils.DOCUMENT_NAME);
		int documentYear = ParamUtil.getInteger(uploadPortletRequest,
				ChuyenDichTrangThaiUtils.DOCUMENT_YEAR);
		int messageType = ParamUtil.getInteger(uploadPortletRequest,
				ChuyenDichTrangThaiUtils.MESSAGE_TYPE);
		int desStatus = ParamUtil.getInteger(uploadPortletRequest,
				ChuyenDichTrangThaiUtils.DES_STATUS);
		int requestState = ParamUtil.getInteger(uploadPortletRequest,
				ChuyenDichTrangThaiUtils.REQUEST_STATE);
		int documentStatusCode = ParamUtil.getInteger(uploadPortletRequest,
				ChuyenDichTrangThaiUtils.DOCUMENT_STATUS_CODE);
		String requestCode = ParamUtil.getString(uploadPortletRequest,
				ChuyenDichTrangThaiUtils.REQUEST_CODE);
		int actionType = ParamUtil.getInteger(uploadPortletRequest,
				ChuyenDichTrangThaiUtils.ACTION_TYPE);
		...
		if (messageType == ChuyenDichTrangThaiUtils.THANH_PHAN_HO_SO) {
			chuyenDichStatus = ChuyenDichTrangThaiUtils.doHoSo(documentName,
					documentYear, desStatus, Boolean.TRUE);
		}
		...
		if (chuyenDichStatus != ChuyenDichTrangThaiUtils.CHUYEN_DICH_THAT_BAI) {
			...
			if (messageType == ChuyenDichTrangThaiUtils.THANH_PHAN_HO_SO) {
				int state = TT1XuLyNghiepVuUtils.doActionHoSo(documentName,
						documentYear, actionType, requestState,
						documentStatusCode, desStatus, themeDisplay.getUser(),
						uploadPortletRequest, actionRequest, Boolean.TRUE);
				...
				TT1XuLyNghiepVuUtils.doActionBanKhai(documentName,
						documentYear, actionType, requestState,
						documentStatusCode, desStatus, themeDisplay.getUser(),
						messageType, requestCode, Boolean.TRUE,
			...
			// documentYear);
			//
			// if (tempDocument.getDocumentStatusCode() ==
			// ChuyenDichTrangThaiUtils.THU_TUC_DA_TIEP_NHAN) {
			//
			// _doCreatePhieuThanhToan(uploadPortletRequest, actionRequest,
			// themeDisplay, documentName,
			// documentYear);
			//
			// }
	...
	public static JSONObject actionThuTuc(ThemeDisplay themeDisplay,
		...
		if (messageType == ChuyenDichTrangThaiUtils.THANH_PHAN_HO_SO) {
			chuyenDichStatus = ChuyenDichTrangThaiUtils.doHoSo(documentName,
					documentYear, desStatus, Boolean.FALSE);
		}
		...
		if (chuyenDichStatus != ChuyenDichTrangThaiUtils.CHUYEN_DICH_THAT_BAI) {
			...
			if (messageType == ChuyenDichTrangThaiUtils.THANH_PHAN_HO_SO) {
				int state = TT1XuLyNghiepVuUtils.doActionHoSo(documentName,
						documentYear, actionType, requestState,
						documentStatusCode, desStatus, themeDisplay.getUser(),
						uploadPortletRequest, actionRequest, Boolean.FALSE);
				// rollback neu xu ly nghiep vu xay ra loi
				if (state == TT1XuLyNghiepVuUtils.XU_LY_THAT_BAI) {
					ChuyenDichTrangThaiUtils.doHoSo(documentName, documentYear,
							documentStatusCode, Boolean.FALSE);
				}
			} else {
				chuyenDichStatus = TT1XuLyNghiepVuUtils.doActionBanKhai(
						documentName, documentYear, actionType, requestState,
						documentStatusCode, desStatus, themeDisplay.getUser(),
						messageType, requestCode, Boolean.FALSE,
						uploadPortletRequest, actionRequest);
			}
			log.info("chuyenDichStatus _doActionBanKhai: " + chuyenDichStatus);
			// create Phieu danh toan
			if (chuyenDichStatus == ChuyenDichTrangThaiUtils.THU_TUC_PHE_DUYET_HOAN_THANH_THU_TUC
					|| chuyenDichStatus == ChuyenDichTrangThaiUtils.THU_TUC_DE_NGHI_CAP_GIAY_PHEP) {
				_doCreatePhieuThanhToan(uploadPortletRequest, actionRequest,
						themeDisplay, documentName, documentYear);
			} else if (chuyenDichStatus == ChuyenDichTrangThaiUtils.THU_TUC_HUY_THU_TUC) {

				long totalpayment = ParamUtil.getLong(uploadPortletRequest,
						"lePhi");
				...
				if (Validator.isNotNull(tempDebitNote)
						&& tempDebitNote.getTotalpayment().doubleValue() > 0) {
					// Tich hop send message 99-30
					boolean resultMethod = TT1TichHopMessageUtils
							.message_99_30(themeDisplay.getUser()
									.getEmailAddress(), uploadPortletRequest,
									actionRequest, documentName, documentYear);
				}
			}
		}
	...
	private static void _doCreatePhieuThanhToan(
			UploadPortletRequest uploadPortletRequest,
		...
		String organization = BusinessUtils
				.getOrganizationFromUser(actionRequest);
		Date fromdate = null;
		int paymenttype = 0;
		...
		if (!VmaItineraryUtils.checkActiveVma(themeDisplay.getUserId())) {
			action.doTempDebitNote(documentName, documentYear, corporationid,
					debitnotenumber, division, financialaccountant, fromdate,
					markasdeleted, organization, paymenttype, reportby,
					reportdate, todate, totalpayment, transactionid,
					themeDisplay.getUser(), uploadPortletRequest, actionRequest);
		}
	}
}
