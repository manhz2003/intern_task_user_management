...
public class TT10JSONProvider {
		...
		try {
			...
			for (Organization organization : orgs) {
				if (organization.getName().equalsIgnoreCase(
						LanguageUtil.get(locale, "ke-hoach"))) {
					...
					objectChilds.put("title",
							LanguageUtil.get(locale, "cho-tiep-nhan-tt"));
					...
					objectChilds.put("title", LanguageUtil.get(locale,
							"yeu-cau-sua-doi-bo-sung-tt"));
					objectChilds.put("type", -1);
					objectChilds.put("counter", "_ _");
					childs.put(objectChilds);
					object.put("items", childs);
					...
					objectChilds.put("title",
							LanguageUtil.get(locale, "tu-choi-tiep-tt"));
					objectChilds.put("type", -1);
					objectChilds.put("counter", "_ _");
					childs.put(objectChilds);
					object.put("items", childs);
					...
					result.put(object);
					// end build ke hoach
				} else if (organization.getName().equalsIgnoreCase(
						LanguageUtil.get(locale, "thu-tuc"))) {
					...
					objectChilds.put("title",
							LanguageUtil.get(locale, "da-tiep-nhan-tt"));
					...
					objectChilds.put("title", LanguageUtil.get(locale,
							"cho-phe-duyet-hoan-thanh-thu-tuc-tt"));
					objectChilds.put("type", -2);
					objectChilds.put("counter", "_ _");
					childs.put(objectChilds);
					object.put("items", childs);
					...
					objectChilds.put("title", LanguageUtil.get(locale,
							"phe-duyet-hoan-thanh-thu-tuc-tt"));
					objectChilds.put("type", -2);
					objectChilds.put("counter", "_ _");
					childs.put(objectChilds);
					object.put("items", childs);
					...
					objectChilds.put("title",
							LanguageUtil.get(locale, "huy-thu-tuc-tt"));
					objectChilds.put("type", -2);
					objectChilds.put("counter", "_ _");
					childs.put(objectChilds);
					object.put("items", childs);
	...
	public static JSONArray getThanhPhanHoSo(ThemeDisplay themeDisplay,
			String documentType, long documentName, int documentYear,
			HttpServletRequest request) {
		JSONArray result = JSONFactoryUtil.createJSONArray();

		JSONObject thongBaoObject = BanKhaiThongBaoUtils.buildThanhPhan(
				themeDisplay, documentName, documentYear,
				MessageType.THONG_BAO_TAU_DEN_CANG);
		JSONObject xacBaoObject = BanKhaiXacBaoUtils.buildThanhPhan(
				themeDisplay, documentName, documentYear,
				MessageType.XAC_BAO_TAU_DEN_CANG);
		JSONObject lenhDieuDongObject = BanKhaiLenhDieuDongUtils
				.buildThanhPhan(themeDisplay, documentName, documentYear);

		if (thongBaoObject.getBoolean("available")/*
												 * && xacBaoObject.getBoolean(
												 * "available")
												 */) {
		...
		// result.put(BanKhaiAnNinhTauBienUtils.buildThanhPhan(themeDisplay,
		// documentName, documentYear));
		// result.put(BanKhaiHangHoaUtils.buildThanhPhan(themeDisplay,
		// documentName, documentYear));
		result.put(thongBaoObject);
		result.put(xacBaoObject);
		// result.put(BanKhaiGiayPhepRoiCangBoGTVTUtils.buildThanhPhan(themeDisplay,
		// documentName, documentYear));
		result.put(lenhDieuDongObject);
		result.put(BanKhaiChungUtils.buildThanhPhan(themeDisplay, documentName,
				documentYear));
		result.put(BanKhaiDanhSachThuyenVienUtils.buildThanhPhan(themeDisplay,
				documentName, documentYear));
		result.put(BanKhaiDanhSachHanhKhachUtils.buildThanhPhan(themeDisplay,
				documentName, documentYear));
		result.put(BanKhaiDanhSachHangHoaNguyHiemUtils.buildThanhPhan(
				themeDisplay, documentName, documentYear));

		// thu-tuc
		try {
			int roleUserFilterselectedType = ParamUtil.getInteger(request,
			...
			for (Organization organization : orgs) {
				if (organization.getName().equalsIgnoreCase(
						LanguageUtil.get(themeDisplay.getLocale(), "thu-tuc"))) {
					isThuTuc = true;
					break;
				} else if (organization.getName().equalsIgnoreCase(
						LanguageUtil.get(themeDisplay.getLocale(), "lanh-dao"))) {
					isThuTuc = true;
					break;
				} else if (organization.getName().equalsIgnoreCase(
						LanguageUtil.get(themeDisplay.getLocale(), "van-thu"))) {
					isThuTuc = true;
					break;
				} else if (organization.getName().equalsIgnoreCase(
						LanguageUtil.get(themeDisplay.getLocale(), "ke-toan"))) {
					isThuTuc = true;
					break;
				}
			}
			...
			if (isThuTuc && roleUserFilterselectedType != -1) {
				result.put(BanKhaiCacGiayToPhaiXuatTrinhUtils.buildThanhPhan(
						themeDisplay, documentName, documentYear));
				result.put(BanKhaiKetQuaPheDuyetHoSoUtils.buildThanhPhan(
						themeDisplay, documentName, documentYear));
			}
		...
		JSONObject lastLost = BanKhaiGiayPhepRoiCangCuoiCungUtils
				.buildThanhPhan(themeDisplay, documentName, documentYear);
	...
	public static JSONArray getFileThanhPhanVersionList(
			ThemeDisplay themeDisplay, int messageType, long documentName,
			int documentYear, HttpServletRequest request) {
		JSONArray result = JSONFactoryUtil.createJSONArray();
		if (messageType == MessageType.THONG_BAO_TAU_DEN_CANG) {

			result = BanKhaiThongBaoUtils.getFileThanhPhanVersionList(
					themeDisplay, documentName, documentYear, messageType);

		} else if (messageType == MessageType.XAC_BAO_TAU_DEN_CANG) {

			result = BanKhaiXacBaoUtils.getFileThanhPhanVersionList(
					themeDisplay, documentName, documentYear, messageType);

		} else if (messageType == MessageType.BAN_KHAI_DANH_SACH_THUYEN_VIEN) {

			result = BanKhaiDanhSachThuyenVienUtils
					.getFileThanhPhanVersionList(themeDisplay, documentName,
							documentYear, messageType);

		} else if (messageType == MessageType.BAN_KHAI_DANH_SACH_HANH_KHACH) {

			result = BanKhaiDanhSachHanhKhachUtils.getFileThanhPhanVersionList(
					themeDisplay, documentName, documentYear, messageType);

		} else if (messageType == MessageType.LENH_DIEU_DONG) {

			result = BanKhaiLenhDieuDongUtils.getFileThanhPhanVersionList(
					themeDisplay, documentName, documentYear, messageType);

		} else if (messageType == MessageType.BAN_KHAI_CHUNG) {

			result = BanKhaiChungUtils.getFileThanhPhanVersionList(
					themeDisplay, documentName, documentYear, messageType);

		} else if (messageType == MessageType.GIAY_PHEP_ROI_CANG_BO_GTVT_CAP_KHI_XUAT_CANH) {

			result = BanKhaiGiayPhepRoiCangBoGTVTUtils
					.getFileThanhPhanVersionList(themeDisplay, documentName,
							documentYear, messageType);

		} else if (messageType == MessageType.GIAY_PHEP_ROI_CANG_CUOI_CUNG) {

			result = BanKhaiGiayPhepRoiCangCuoiCungUtils
					.getFileThanhPhanVersionList(themeDisplay, documentName,
							documentYear, messageType);
	...
	public static JSONObject exportPDFDetailThanhPhan(Locale locale,
			int documentName, int documentYear, int messageType,
		...
		try {
			...
			long nanoTime = ReportsBusinessUtils.actionExport(requestCode,
			...
			if (messageType == MessageType.LENH_DIEU_DONG) {
				shiftOrder = IssueShiftingOrderLocalServiceUtil
						.getByRequestCode(requestCode);
			}

			if (messageType == MessageType.XAC_BAO_TAU_DEN_CANG) {
				tenFileExport = nanoTime + "_"
						+ ReportConstant.XACBAO_NOTICE_OF_ARRIVAL_EXPORT;
			} else if (messageType == MessageType.THONG_BAO_TAU_DEN_CANG) {
				tenFileExport = nanoTime + "_"
						+ ReportConstant.NOTICE_OF_ARRIVAL_EXPORT;
			} else if (messageType == MessageType.LENH_DIEU_DONG) {
				if (Validator.isNotNull(shiftOrder)
						&& shiftOrder.getAttachedFile() > 0
						&& DocumentUtils.existDLFileAbsPath(shiftOrder
								.getAttachedFile())) {
					tenFileExport = vn.gt.portlet.document.FileUtils
							.getFullFileURL(shiftOrder.getAttachedFile());
				} else {
					tenFileExport = nanoTime + "_"
							+ ReportConstant.SHIFTING_ORDER_EXPORT;
				}
			} else if (messageType == MessageType.BAN_KHAI_CHUNG) {
				tenFileExport = nanoTime + "_"
						+ ReportConstant.GENERAL_DECLARATION_EXPORT;
			} else if (messageType == MessageType.BAN_KHAI_DANH_SACH_THUYEN_VIEN) {
				tenFileExport = nanoTime + "_"
						+ ReportConstant.CREW_LIST_EXPORT;
			} else if (messageType == MessageType.BAN_KHAI_DANH_SACH_HANH_KHACH) {
				tenFileExport = nanoTime + "_"
						+ ReportConstant.PASSENGER_LIST_EXPORT;
			} else if (messageType == MessageType.GIAY_PHEP_ROI_CANG_CUOI_CUNG) {
				...
				TempGeneralDeclaration tempGeneralDeclaration = TempGeneralDeclarationLocalServiceUtil
				...
				if (Validator.isNotNull(tempGeneralDeclaration)) {
					...
					if (Validator.isNotNull(tempNoTiceShipMessage)) {
						portCleaneNo = tempNoTiceShipMessage
								.getPortClearanceNo();
					}
				}
				IssuePortClearance issuePortClearance = IssuePortClearanceLocalServiceUtil
						.findLatestCertificateOfIssuePortClearance(null, null,
								portCleaneNo);

				if (Validator.isNotNull(issuePortClearance)) {
					if (issuePortClearance.getAttachedFile() > 0
							&& DocumentUtils
									.existDLFileAbsPath(issuePortClearance
											.getAttachedFile())) {
						tenFileExport = vn.gt.portlet.document.FileUtils
								.getFullFileURL(issuePortClearance
										.getAttachedFile());
						flagGiayPhepRoiCangCC = true;
					} else {
						long documentNameTemp = issuePortClearance
								.getDocumentName();
						long documentYearTemp = issuePortClearance
								.getDocumentYear();
						TempDocument temp = TempDocumentLocalServiceUtil
								.getByDocumentNameAndDocumentYear(
										documentNameTemp,
										Integer.valueOf(documentYearTemp + ""));
						nanoTime = ReportsBusinessUtils
								.actionExport(
										issuePortClearance.getRequestCode(),
										Integer.valueOf(documentNameTemp + ""),
										Integer.valueOf(documentYearTemp + ""),
										MessageType.GIAY_PHEP_ROI_CANG_BO_GTVT_CAP_KHI_XUAT_CANH,
										temp.getDocumentTypeCode());
						log.info("-------documentNameTemp+++++++++++"
								+ documentNameTemp);
						log.info("-------documentYearTemp+++++++++++"
								+ documentYearTemp);
						log.info("-------nanoTime+++++++++++" + nanoTime);
						log.info("-------getRequestCode+++++++++++"
								+ issuePortClearance.getRequestCode());
						tenFileExport = nanoTime + "_"
								+ ReportConstant.ISSUE_PORT_CLEARANCE_EXPORT;
			...
			if ((Validator.isNotNull(shiftOrder)
					&& shiftOrder.getAttachedFile() > 0
					&& DocumentUtils.existDLFileAbsPath(shiftOrder
							.getAttachedFile()) && messageType == MessageType.LENH_DIEU_DONG)) {
				result.put("url", tenFileExport);
				result.put("download", tenFileExport);
			} else if (flagGiayPhepRoiCangCC
					&& Validator.isNotNull(tenFileExport)
	...
	public static JSONObject checkThanhPhanActionButton(
		...
		try {
			...
			if (roleType == -1) {
				...
				if (messageType == 70) {
					...
					List<DmRepresentative> listRepres = DmRepresentativeLocalServiceUtil
							.findByMaritimeCode(userPort.getPortCode());
					...
					for (DmRepresentative dmRepresentative : listRepres) {
						...
						if (dmRepresentative.getRepLevel() == 1 || dmRepresentative.getRepLevel() == 2 || dmRepresentative.getRepLevel() == 4) {
							...
							userObj.put("representative",
									dmRepresentative.getRepName());
					...
					if (lstShiftingOrder.size() > 0) {
						...
						if (item.getRequestState() == TrangThaiBanKhaiNhapCanh.KHAI_SUA
								&& temp.getRequestState() == ChuyenDichTrangThaiUtils.KE_HOACH_CHO_SUA_LENH_DIEU_DONG) {
							resultShow = 2;
							// break;
						}
						// }
			...
			resultShow = CheckBusinessUtil.checkRoleDisplayButton(
					ChuyenDichTrangThaiUtils.CHUYEN_CANG_VAO, messageType,
					vaiTro, temp, requestCode, actionRequest);
