...
public class BC12SSTExport {
	...
	public static JSONObject getModel(String maritimeCode, String fromDate,
		...
		// =============================== Luot_HH
		BC12BTLuotHHModel dauNamDenHetThangTruoc_bc12btLuotHHModel = VmaItineraryLocalServiceUtil
				.getModelMau12BTLuotHH(maritimeCode,
						dauNamDenThangTruoc_startDate,
						dauNamDenThangTruoc_endDate);
		...
		// ========================== Khu_Cang
		List<BC12BTKhuCangModel> dauNamDenHetThangTruoc_bc12btKhuCangModels = DmPortRegionLocalServiceUtil
		...
		// ========================== Vinalines
		List<BC12BTVinalinesModel> dauNamDenHetThangTruoc_bc12btVinalinesModels = DmPortWharfLocalServiceUtil
		...
		JSONObject c = object2Json(dauNamDenHetThangTruoc_bc12btLuotHHModel,
				"C");
		JSONObject d = object2Json(thangBaoCao_bc12btLuotHHModel, "D");
		JSONObject e = object2Json(thangBaoCaoNamTruoc_bc12btLuotHHModel, "E");
		JSONObject f = object2Json(soVoiCungKy_bc12btLuotHHModel, "F");
		JSONObject g = object2Json(luyKeDauNamDenHetThangBC_bc12btLuotHHModel,
				"G");
		JSONObject h = object2Json(luyKeCungKyNamTruoc_bc12btLuotHHModel, "H");
		JSONObject i = object2Json(luyKeSoVoiCungKy_bc12btLuotHHModel, "I");

		JSONObject result = BaoCaoBussinessUtils.mergeJson(c, d, e, f, g, h, i);

		// ====================================== khoiLuongHH
		// Neu khong cai nao co du lieu thi sao
		int sizeKhuCang[] = {
				dauNamDenHetThangTruoc_bc12btKhuCangModels.size(),
				thangBaoCao_bc12btKhuCangModels.size(),
		...
		double tong_khoi_luong_hang_thangbc = 0, tong_khoi_luong_hang_daunamdenhetthangbc = 0, tong_khoi_luong_hang_thangbcnamtruoc = 0, tong_khoi_luong_hang_sovoicungky = 0, tong_khoi_luong_hang_luykedaunamdenhetthangbc = 0, tong_khoi_luong_hang_luykecungkynamtruoc = 0, tong_khoi_luong_hang_luykesovoicungky = 0;
		...
		for (int count = 0; count < max; count++) {
			...
			try {
				obj.put("C",
						dauNamDenHetThangTruoc_bc12btKhuCangModels.get(count)
								.getKhoi_luong_hang() >= 0 ? formatter
								.format(dauNamDenHetThangTruoc_bc12btKhuCangModels
			...
			tong_khoi_luong_hang_daunamdenhetthangbc += dauNamDenHetThangTruoc_bc12btKhuCangModels
					.get(count).getKhoi_luong_hang() >= 0 ? dauNamDenHetThangTruoc_bc12btKhuCangModels
			...
			if (count == max) {
				result.put("C27", formatter
						.format(tong_khoi_luong_hang_daunamdenhetthangbc));
				result.put("D27",
		...
		for (int count = 0; count < max; count++) {
			...
			try {
				obj.put("C",
						dauNamDenHetThangTruoc_bc12btKhuCangModels.get(count)
								.getLuot_tau() >= 0 ? dauNamDenHetThangTruoc_bc12btKhuCangModels
								.get(count).getLuot_tau() : null);
			} catch (Exception ex) {

			}
			try {
				obj.put("D", thangBaoCao_bc12btKhuCangModels.get(count)
			...
			try {
				obj.put("F", soVoiCungKy_bc12btKhuCangModels.get(count)
			...
			try {
				luot_tau_thuyen_daunamdenhetthangbc += dauNamDenHetThangTruoc_bc12btKhuCangModels
						.get(count).getLuot_tau() >= 0 ? dauNamDenHetThangTruoc_bc12btKhuCangModels
			...
			if (count == max) {
				result.put("C28", luot_tau_thuyen_daunamdenhetthangbc);
				result.put("D28", luot_tau_thuyen_thangbc);
				result.put("E28", luot_tau_thuyen_thangbcnamtruoc);
				result.put("F28", luot_tau_thuyen_sovoicungky);
				result.put("G28", luot_tau_thuyen_luykedaunamdenhetthangbc);
				result.put("H28", luot_tau_thuyen_luykecungkynamtruoc);
				result.put("I28", luot_tau_thuyen_luykesovoicungky);
			}
		...
		// ================================//=========================================
		double luot_tau_bien_thangbc = 0, luot_tau_bien_daunamdenhetthangbc = 0, luot_tau_bien_thangbcnamtruoc = 0, luot_tau_bien_sovoicungky = 0, luot_tau_bien_luykedaunamdenhetthangbc = 0, luot_tau_bien_luykecungkynamtruoc = 0, luot_tau_bien_luykesovoicungky = 0;
		JSONArray arrayLuotTauBien = JSONFactoryUtil.createJSONArray();
		...
		for (int count = 0; count < max; count++) {
			...
			try {
				obj.put("C",
						dauNamDenHetThangTruoc_bc12btKhuCangModels.get(count)
								.getLuot_tau_bien() >= 0 ? dauNamDenHetThangTruoc_bc12btKhuCangModels
			...
			try {
				obj.put("D",
						thangBaoCao_bc12btKhuCangModels.get(count)
								.getLuot_tau_bien() >= 0 ? thangBaoCao_bc12btKhuCangModels
								.get(count).getLuot_tau_bien() : null);
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				obj.put("E",
						thangBaoCaoNamTruoc_bc12btKhuCangModels.get(count)
								.getLuot_tau_bien() >= 0 ? thangBaoCaoNamTruoc_bc12btKhuCangModels
								.get(count).getLuot_tau_bien() : null);
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				obj.put("F",
						soVoiCungKy_bc12btKhuCangModels.get(count)
								.getLuot_tau_bien() >= 0 ? soVoiCungKy_bc12btKhuCangModels
								.get(count).getLuot_tau_bien() : null);
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				obj.put("G",
						luyKeDauNamDenHetThangBC_bc12btKhuCangModels.get(count)
								.getLuot_tau_bien() >= 0 ? luyKeDauNamDenHetThangBC_bc12btKhuCangModels
								.get(count).getLuot_tau_bien() : null);
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				obj.put("H",
						luyKeCungKyNamTruoc_bc12btKhuCangModels.get(count)
								.getLuot_tau_bien() >= 0 ? luyKeCungKyNamTruoc_bc12btKhuCangModels
								.get(count).getLuot_tau_bien() : null);
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				obj.put("I",
						luyKeSoVoiCungKy_bc12btKhuCangModels.get(count)
								.getLuot_tau_bien() >= 0 ? luyKeSoVoiCungKy_bc12btKhuCangModels
								.get(count).getLuot_tau_bien() : null);
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				luot_tau_bien_daunamdenhetthangbc += dauNamDenHetThangTruoc_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() >= 0 ? dauNamDenHetThangTruoc_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() : 0;
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				luot_tau_bien_thangbc += thangBaoCao_bc12btKhuCangModels.get(
						count).getLuot_tau_bien() >= 0 ? thangBaoCao_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() : 0;
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				luot_tau_bien_thangbcnamtruoc += thangBaoCaoNamTruoc_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() >= 0 ? thangBaoCaoNamTruoc_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() : 0;
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				luot_tau_bien_sovoicungky += soVoiCungKy_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() >= 0 ? soVoiCungKy_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() : 0;
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				luot_tau_bien_luykedaunamdenhetthangbc += luyKeDauNamDenHetThangBC_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() >= 0 ? luyKeDauNamDenHetThangBC_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() : 0;
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				luot_tau_bien_luykecungkynamtruoc += luyKeCungKyNamTruoc_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() >= 0 ? luyKeCungKyNamTruoc_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() : 0;
			} catch (Exception ex) {
				// TODO: handle exception
			}
			try {
				luot_tau_bien_luykesovoicungky += luyKeSoVoiCungKy_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() >= 0 ? luyKeSoVoiCungKy_bc12btKhuCangModels
						.get(count).getLuot_tau_bien() : 0;
			} catch (Exception ex) {
				// TODO: handle exception
			}
			...
			if (count == max) {
				result.put("C29", luot_tau_bien_daunamdenhetthangbc);
				result.put("D29", luot_tau_bien_thangbc);
				result.put("E29", luot_tau_bien_thangbcnamtruoc);
				result.put("F29", luot_tau_bien_sovoicungky);
				result.put("G29", luot_tau_bien_luykedaunamdenhetthangbc);
				result.put("H29", luot_tau_bien_luykecungkynamtruoc);
				result.put("I29", luot_tau_bien_luykesovoicungky);
			}

			arrayLuotTauBien.put(obj);
		}
		result.put("Bang3", arrayLuotTauBien);

		// =========================//================================================
		double luot_pttnd_thangbc = 0, luot_pttnd_daunamdenhetthangbc = 0, luot_pttnd_thangbcnamtruoc = 0, luot_pttnd_sovoicungky = 0, luot_pttnd_luykedaunamdenhetthangbc = 0, luot_pttnd_luykecungkynamtruoc = 0, luot_pttnd_luykesovoicungky = 0;
		JSONArray arrayLuotPttnd = JSONFactoryUtil.createJSONArray();
		...
		for (int count = 0; count < max; count++) {
			...
			try {
				obj.put("C",
						dauNamDenHetThangTruoc_bc12btKhuCangModels.get(count)
								.getLuot_pttnd() >= 0 ? dauNamDenHetThangTruoc_bc12btKhuCangModels
			...
			try {
				obj.put("D", thangBaoCao_bc12btKhuCangModels.get(count)
						.getLuot_pttnd() >= 0 ? thangBaoCao_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : null);
			} catch (Exception ex) {
			...
			try {
				obj.put("E",
						thangBaoCaoNamTruoc_bc12btKhuCangModels.get(count)
								.getLuot_pttnd() >= 0 ? thangBaoCaoNamTruoc_bc12btKhuCangModels
								.get(count).getLuot_pttnd() : null);
			} catch (Exception ex) {
			...
			try {
				obj.put("F", soVoiCungKy_bc12btKhuCangModels.get(count)
						.getLuot_pttnd() >= 0 ? soVoiCungKy_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : null);
			} catch (Exception ex) {
			...
			try {
				obj.put("G",
						luyKeDauNamDenHetThangBC_bc12btKhuCangModels.get(count)
								.getLuot_pttnd() >= 0 ? luyKeDauNamDenHetThangBC_bc12btKhuCangModels
								.get(count).getLuot_pttnd() : null);
			} catch (Exception ex) {
			...
			try {
				obj.put("H",
						luyKeCungKyNamTruoc_bc12btKhuCangModels.get(count)
								.getLuot_pttnd() >= 0 ? luyKeCungKyNamTruoc_bc12btKhuCangModels
								.get(count).getLuot_pttnd() : null);
			} catch (Exception ex) {
			...
			try {
				obj.put("I",
						luyKeSoVoiCungKy_bc12btKhuCangModels.get(count)
								.getLuot_pttnd() >= 0 ? luyKeSoVoiCungKy_bc12btKhuCangModels
								.get(count).getLuot_pttnd() : null);
			} catch (Exception ex) {

			}
			try {
				luot_pttnd_daunamdenhetthangbc += dauNamDenHetThangTruoc_bc12btKhuCangModels
						.get(count).getLuot_pttnd() >= 0 ? dauNamDenHetThangTruoc_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : 0;
			} catch (Exception ex) {

			}
			try {
				luot_pttnd_thangbc += thangBaoCao_bc12btKhuCangModels
						.get(count).getLuot_pttnd() >= 0 ? thangBaoCao_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : 0;
			} catch (Exception ex) {

			}
			try {
				luot_pttnd_thangbcnamtruoc += thangBaoCaoNamTruoc_bc12btKhuCangModels
						.get(count).getLuot_pttnd() >= 0 ? thangBaoCaoNamTruoc_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : 0;
			} catch (Exception ex) {

			}
			try {
				luot_pttnd_sovoicungky += soVoiCungKy_bc12btKhuCangModels.get(
						count).getLuot_pttnd() >= 0 ? soVoiCungKy_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : 0;
			} catch (Exception ex) {

			}
			try {
				luot_pttnd_luykedaunamdenhetthangbc += luyKeDauNamDenHetThangBC_bc12btKhuCangModels
						.get(count).getLuot_pttnd() >= 0 ? luyKeDauNamDenHetThangBC_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : 0;
			} catch (Exception ex) {

			}
			try {
				luot_pttnd_luykecungkynamtruoc += luyKeCungKyNamTruoc_bc12btKhuCangModels
						.get(count).getLuot_pttnd() >= 0 ? luyKeCungKyNamTruoc_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : 0;
			} catch (Exception ex) {

			}
			try {
				luot_pttnd_luykesovoicungky += luyKeSoVoiCungKy_bc12btKhuCangModels
						.get(count).getLuot_pttnd() >= 0 ? luyKeSoVoiCungKy_bc12btKhuCangModels
						.get(count).getLuot_pttnd() : 0;
			} catch (Exception ex) {
			...
			if (count == max) {
				result.put("C30", luot_pttnd_daunamdenhetthangbc);
				result.put("D30", luot_pttnd_thangbc);
				result.put("E30", luot_pttnd_thangbcnamtruoc);
				result.put("F30", luot_pttnd_sovoicungky);
				result.put("G30", luot_pttnd_luykedaunamdenhetthangbc);
				result.put("H30", luot_pttnd_luykecungkynamtruoc);
				result.put("I30", luot_pttnd_luykesovoicungky);
			}

			arrayLuotPttnd.put(obj);
		}
		result.put("Bang4", arrayLuotPttnd);

		// ========================= Vinalines =======================
		int sizeVinalines[] = {
				dauNamDenHetThangTruoc_bc12btVinalinesModels.size(),
				thangBaoCao_bc12btVinalinesModels.size(),
		...
		for (int count = 0; count < max; count++) {
			...
			try {
				obj.put("C2",
						dauNamDenHetThangTruoc_bc12btVinalinesModels.get(count)
								.getHang_pttnd() >= 0 ? formatter
								.format(dauNamDenHetThangTruoc_bc12btVinalinesModels
				...
				obj.put("C3",
						dauNamDenHetThangTruoc_bc12btVinalinesModels.get(count)
								...
								: null);
				double tong1 = BaoCaoBussinessUtils.sum1(
						dauNamDenHetThangTruoc_bc12btVinalinesModels.get(count)
								.getHang_pttnd(),
						dauNamDenHetThangTruoc_bc12btVinalinesModels.get(count)
								.getHang_tau_bien());
				obj.put("C1", tong1 >= 0 ? formatter.format(tong1) : null);
			} catch (Exception ex) {
				// nothing to do
			}
			try {
			...
			try {
				...
				double tong3 = BaoCaoBussinessUtils.sum1(
						...
						thangBaoCaoNamTruoc_bc12btVinalinesModels.get(count)
								.getHang_tau_bien());
				obj.put("E1", tong3 >= 0 ? formatter.format(tong3) : null);
			} catch (Exception ex) {
				// nothing to do
			}
			try {
			...
			try {
				...
				obj.put("G1", tong5 >= 0 ? formatter.format(tong5) : null);
			} catch (Exception ex) {
				// nothing to do
			}
			try {
				...
				double tong6 = BaoCaoBussinessUtils.sum1(
						...
						luyKeCungKyNamTruoc_bc12btVinalinesModels.get(count)
								.getHang_tau_bien());
				obj.put("H1", tong6 >= 0 ? formatter.format(tong6) : null);
			} catch (Exception ex) {
				// nothing to do
			}
			try {
				...
				double tong7 = BaoCaoBussinessUtils.sum1(
						...
						luyKeSoVoiCungKy_bc12btVinalinesModels.get(count)
								.getHang_tau_bien());
				obj.put("I1", tong7 >= 0 ? formatter.format(tong7) : null);
			} catch (Exception ex) {
				// nothing to do
			}
		...
		result.put("Bang5", arrayHangVinalines);
		...
		for (int count = 0; count < max; count++) {
			...
			try {
				obj.put("C2",
						dauNamDenHetThangTruoc_bc12btVinalinesModels.get(count)
								.getLuot_pttnd() >= 0 ? dauNamDenHetThangTruoc_bc12btVinalinesModels
								.get(count).getLuot_pttnd() : null);
				obj.put("C3",
						dauNamDenHetThangTruoc_bc12btVinalinesModels.get(count)
								.getLuot_tau_bien() >= 0 ? dauNamDenHetThangTruoc_bc12btVinalinesModels
								.get(count).getLuot_tau_bien() : null);
				int tong1 = BaoCaoBussinessUtils.sum2(
						dauNamDenHetThangTruoc_bc12btVinalinesModels.get(count)
								.getLuot_pttnd(),
						dauNamDenHetThangTruoc_bc12btVinalinesModels.get(count)
								.getLuot_tau_bien());
				obj.put("C1", tong1 >= 0 ? tong1 : null);
			} catch (Exception ex) {
				// nothing to do
			}
			try {
		...
		result.put("Bang6", arrayLuotTauVinalines);
	...
	public static JSONObject object2Json(BC12BTLuotHHModel obj, String objName) {
		...
		if (obj.getLuot_tau_bien_duoi_200() >= 0) {
			result.put(objName + "2_1", obj.getLuot_tau_bien_duoi_200());
		}
		if (obj.getLuot_tau_bien_duoi_200() >= 0) {
			result.put(objName + "2_2", obj.getLuot_tau_bien_tren_200());
		}
		...
		if (obj.getTong_hang_pttnd() >= 0) {
			result.put(objName + "16", obj.getTong_hang_pttnd()); // Khong biet
																	// truong
																	// nao
		}
		if (obj.getTong_hang_pttnd() >= 0) {
			result.put(objName + "17", obj.getTong_hang_pttnd()); // Khong biet
																	// truong
																	// nao
		}
		...
		if (sum_hh_pttnd >= 0) {
			result.put(objName + "18", formatter.format(sum_hh_pttnd));
		}
		...
		if (obj.getLuot_tau_bien() >= 0) {
			result.put(objName + "23", obj.getLuot_tau_bien()); // Khong biet
																// truong nao
		}
		...
		if (obj.getTong_hang_qua_canh() >= 0) {
			result.put(objName + "31",
					formatter.format(obj.getTong_hang_qua_canh()));
		}
		if (obj.getLuot_khach_tau_bien_vn() >= 0) {
			result.put(objName + "32", obj.getLuot_khach_tau_bien_vn());
		}
		if (obj.getLuot_khach_tau_bien_nn() >= 0) {
			result.put(objName + "33", obj.getLuot_khach_tau_bien_nn());
		}
		if (obj.getLuot_khach_pttnd() >= 0) {
			result.put(objName + "34", obj.getLuot_khach_pttnd());
		}
		if (obj.getKhach_tu_bo_ra_dao() >= 0) {
			result.put(objName + "35", obj.getKhach_tu_bo_ra_dao());
		}
