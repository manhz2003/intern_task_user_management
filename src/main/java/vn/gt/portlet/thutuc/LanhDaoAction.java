...
(WebKeys.THEME_DISPLAY);
			...
			if (fileEntryId > 0) {
				...
				DLFileEntry fileEntry = FileUploadUtils.uploadFile(themeDisplay.getUserId(), groupId, fileEntryId, file,
				...
				if (advSearch) {
					...
					return writeJSON(resourceRequest, resourceResponse,
							JSONProviderUtils.getHistoryTable(themeDisplay, documentTypeCode, maritimeCodeOwner,
									...
									daily, maritimeCodeNext, start, end, isDTND, isDTNDCam, daKy.trim()));
				} else {
					return writeJSON(resourceRequest, resourceResponse,
